...
function templateToMeals(template: DietTemplate): Meal[] {
  return (template.meals ?? []).map((tm: DietTemplateMeal) => ({
    plan_id:         0,
...
export function TemplateImportModal({ open, hasMeals, onClose, onImport }: Props) {
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] flex flex-col gap-0 p-0 overflow-hidden">
        ...
        <DialogHeader className="px-6 pt-5 pb-4 border-b border-border/60 shrink-0">
          <DialogTitle className="flex items-center gap-2 text-base">
            ...
            Importar Template de Cardápio
          </DialogTitle>
          <p className="text-xs text-muted-foreground mt-1">
            Selecione um template para preencher automaticamente as refeições e alimentos do plano.
          </p>
        </DialogHeader>
            ...
            <>
              ...
              {selected && (
                <div className="rounded-lg border border-border/60 overflow-hidden mt-2">
                  ...
                  >
                    <span>Prévia das refeições — {selected.meals?.length ?? 0} refeições</span>
              ...
              {selected && hasMeals && (
                <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 space-y-2">
                  ...
                  <div className="flex flex-col gap-1.5">
                    {([
                      ["replace", "Substituir — apagar as refeições atuais e importar o template"],
                      ["append",  "Adicionar — manter as refeições atuais e acrescentar as do template"],
                    ] as [typeof mode, string][]).map(([val, label]) => (
