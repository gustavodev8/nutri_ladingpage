...
export function FoodSearchInput({ value, onSelect, onCustomName }: FoodSearchInputProps) {
  ...
  const handleSelect = (food: FoodItem) => {
    setQuery(food.name);
    setIsOpen(false);
    onSelect({ name: food.name, kcal_per_100g: food.kcal, protein_per_100g: food.protein, carbs_per_100g: food.carbs, fat_per_100g: food.fat });
  };
  ...
  const openModal = () => { setForm({ ...EMPTY_FORM, name: query }); setFormErrors({}); setIsOpen(false); setShowModal(true); };
  const closeModal = () => { setShowModal(false); setForm(EMPTY_FORM); setFormErrors({}); };
  ...
  const validateForm = (): boolean => {
    ...
    if (!form.kcal_per_100g || isNaN(Number(form.kcal_per_100g))) errors.kcal_per_100g = "Informe as calorias";
    if (!form.protein_per_100g || isNaN(Number(form.protein_per_100g))) errors.protein_per_100g = "Informe a proteína";
    if (!form.carbs_per_100g || isNaN(Number(form.carbs_per_100g))) errors.carbs_per_100g = "Informe os carboidratos";
    if (!form.fat_per_100g || isNaN(Number(form.fat_per_100g))) errors.fat_per_100g = "Informe a gordura";
  ...
  const handleSaveCustomFood = async () => {
    ...
    try {
      const saved = await saveCustomFood({
        name: form.name.trim(), category: form.category || "Personalizado",
        kcal_per_100g: Number(form.kcal_per_100g), protein_per_100g: Number(form.protein_per_100g),
        carbs_per_100g: Number(form.carbs_per_100g), fat_per_100g: Number(form.fat_per_100g),
        fiber_per_100g: form.fiber_per_100g ? Number(form.fiber_per_100g) : undefined,
      });
      setQuery(saved.name);
      closeModal();
      onSelect({ name: saved.name, kcal_per_100g: saved.kcal_per_100g, protein_per_100g: saved.protein_per_100g, carbs_per_100g: saved.carbs_per_100g, fat_per_100g: saved.fat_per_100g });
        ...
        {isOpen && dropdownPos && createPortal(
          ...
          >
            ...
            <div className="border-t border-border divide-y divide-border/60">
              <button type="button" onMouseDown={(e) => { e.preventDefault(); handleFreeText(); }}
                ...
                <span className="truncate text-left">
                  Usar <span className="font-semibold">"{query}"</span> como texto livre{" "}
                  <span className="text-muted-foreground text-xs">(sem macros)</span>
                </span>
              </button>
              <button type="button" onMouseDown={(e) => { e.preventDefault(); openModal(); }}
                ...
                <span className="truncate">
                  Cadastrar <span className="font-semibold">"{query}"</span> com macronutrientes
                </span>
              </button>
            </div>
          </div>,
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm"
          ...
          <div className="bg-background border border-border rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-border">
              ...
              <button type="button" onClick={closeModal} className="text-muted-foreground hover:text-foreground transition-colors" aria-label="Fechar">
                <X className="h-5 w-5" />
              </button>
            </div>
            <div className="px-6 py-5 space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-sm font-medium text-foreground">Nome do alimento <span className="text-destructive">*</span></label>
                  <input type="text" value={form.name} onChange={(e) => setField("name", e.target.value)}
                    placeholder="Ex: Pão de queijo caseiro"
                    className={cn("flex h-10 w-full rounded-lg border bg-background px-3 py-2 text-sm",
                      "ring-offset-background placeholder:text-muted-foreground",
                      "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2",
                ...
                <div className="space-y-1.5">
                  ...
                  <select value={form.category} onChange={(e) => setField("category", e.target.value)}
                    className={cn("flex h-10 w-full rounded-lg border border-input bg-background px-3 py-2 text-sm",
            ...
            <div className="flex items-center justify-end gap-3 px-6 pb-6 pt-2">
              <button type="button" onClick={closeModal} disabled={isSaving}
                className={cn("h-9 px-4 rounded-lg border border-input bg-background text-sm font-medium text-foreground",
                  "hover:bg-muted/60 transition-colors disabled:opacity-50 disabled:cursor-not-allowed")}>Cancelar</button>
              <button type="button" onClick={handleSaveCustomFood} disabled={isSaving}
                className={cn("h-9 px-5 rounded-lg bg-primary text-primary-foreground text-sm font-medium",
                  "hover:bg-primary/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2")}>
                {isSaving ? (<><span className="h-3.5 w-3.5 rounded-full border-2 border-primary-foreground/40 border-t-primary-foreground animate-spin" />Salvando...</>) : "Salvar alimento"}
              </button>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="space-y-1.5">
      ...
      <input type="number" min="0" step="0.1" value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder}
        className={cn("flex h-9 w-full rounded-lg border bg-background px-3 py-2 text-sm",
          "ring-offset-background placeholder:text-muted-foreground",
          "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2",
