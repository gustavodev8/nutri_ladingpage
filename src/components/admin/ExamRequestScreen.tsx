    ...
    <div className="space-y-5">
      <div className="space-y-2">
        ...
        <div className="flex flex-wrap gap-2">
          <button type="button" onClick={() => handleProtocolChange(null)}
            className={cn("px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors",
              selectedProtocolId === null ? "bg-primary text-primary-foreground border-primary" : "border-border text-muted-foreground hover:bg-muted")}>
            Seleção manual
          </button>
          {protocols.map((p) => (
            <button key={p.id} type="button" onClick={() => handleProtocolChange(p.id)}
              className={cn("px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors",
                selectedProtocolId === p.id ? "bg-primary text-primary-foreground border-primary" : "border-border text-muted-foreground hover:bg-muted")}>
              {p.name}
            </button>
          ))}
        </div>
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-1.5">
          <span className="text-xs font-semibold text-foreground">{selectedExamIds.size} exame(s) selecionado(s)</span>
          {selectedExamIds.size > 0 && (
            <button type="button" onClick={() => setSelectedExamIds(new Set())}
              className="text-[11px] text-muted-foreground hover:text-destructive underline transition-colors">limpar</button>
          )}
        </div>
        <div className="relative w-52">
          <Search size={12} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-muted-foreground" />
          <Input ref={searchRef} value={search} onChange={(e) => setSearch(e.target.value)}
            placeholder="Buscar exame…" className="pl-7 h-8 text-sm" />
        </div>
      </div>
      <div className="space-y-3 max-h-[440px] overflow-y-auto pr-1">
        {Object.entries(grouped).map(([category, exams]) => {
          ...
          return (
            <div key={category} className="rounded-lg border border-border/50 overflow-hidden">
              <div className="flex items-center justify-between px-3 py-2 bg-muted/30 cursor-pointer select-none hover:bg-muted/50 transition-colors"
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] font-bold uppercase tracking-wider text-foreground">{category}</span>
                  <span className="text-[10px] text-muted-foreground">({exams.filter((e) => selectedExamIds.has(e.id)).length}/{exams.length})</span>
                </div>
                <button type="button" onClick={(ev) => { ev.stopPropagation(); toggleAll(exams, !allSelected); }}
                  className={cn("text-[10px] font-semibold transition-colors px-2 py-0.5 rounded",
                    someSelected ? "text-primary hover:text-primary/80" : "text-muted-foreground hover:text-foreground")}>
                  ...
                  {exams.map((exam) => {
                    ...
                    return (
                      <button key={exam.id} type="button" onClick={() => toggleExam(exam.id)}
                        className={cn("flex items-center gap-2.5 px-3 py-2 rounded-md text-left transition-all",
                          selected ? "bg-primary/8 text-foreground" : "text-muted-foreground hover:bg-muted/60 hover:text-foreground")}>
                        <span className={cn("flex-shrink-0 w-3.5 h-3.5 rounded border flex items-center justify-center transition-colors",
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {exam.unit && <p className="text-[10px] text-muted-foreground">{exam.unit}</p>}
                        </div>
                      </button>
                    );
                  })}
        ...
        {Object.keys(grouped).length === 0 && (
          <p className="text-sm text-muted-foreground text-center py-6">Nenhum exame encontrado para "{search}".</p>
        )}
      </div>
      <div className="space-y-1.5">
        ...
        <Input value={notes} onChange={(e) => setNotes(e.target.value)}
          placeholder="Ex: jejum de 12h, não usar biotina 48h antes…" className="text-sm" />
      </div>
      <div className="flex items-center justify-end gap-2.5 pt-1">
        <Button variant="outline" size="sm" onClick={onCancel}>Cancelar</Button>
        <Button size="sm" onClick={handleSubmit} disabled={saving || selectedExamIds.size === 0} className="gap-1.5">
