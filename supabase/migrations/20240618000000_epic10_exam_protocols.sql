...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM exam_protocols LIMIT 1) THEN
    ...
    SELECT p1, id, ROW_NUMBER() OVER (ORDER BY id) - 1 FROM exams_catalog WHERE name IN (
      ...
      'TGO (AST)','TGP (ALT)','Gama-GT','Creatinina','Ureia','Ácido Úrico',
      'Hemoglobina','Leucócitos','Plaquetas','Ferritina','TSH','T4 Livre',
      'Vitamina D (25-OH)','Vitamina B12','PCR Ultrassensível');
    INSERT INTO protocol_exams (protocol_id, exam_id, sort_order)
    SELECT p2, id, ROW_NUMBER() OVER (ORDER BY id) - 1 FROM exams_catalog WHERE name IN (
      ...
      'TSH','T4 Livre','T3 Livre','Vitamina D (25-OH)','Vitamina B12','Magnésio','Zinco',
      'PCR Ultrassensível','Homocisteína','Ferritina','Cortisol (Manhã)','DHEA-S',
      'TGO (AST)','TGP (ALT)');
    INSERT INTO protocol_exams (protocol_id, exam_id, sort_order)
    SELECT p3, id, ROW_NUMBER() OVER (ORDER BY id) - 1 FROM exams_catalog WHERE name IN (
      'Testosterona Total','Testosterona Livre','DHEA-S','Cortisol (Manhã)',
      'IGF-1','LH','FSH','Prolactina','Glicemia de Jejum','Insulina de Jejum','HOMA-IR',
      'Hemoglobina','Ferritina','Ferro Sérico','Vitamina D (25-OH)','Vitamina B12',
      'Zinco','Magnésio','PCR Ultrassensível','Albumina','TGO (AST)','TGP (ALT)','Creatinina');
    INSERT INTO protocol_exams (protocol_id, exam_id, sort_order)
    SELECT p4, id, ROW_NUMBER() OVER (ORDER BY id) - 1 FROM exams_catalog WHERE name IN (
      'TSH','T4 Livre','T3 Livre','T3 Total','Anti-TPO',
      'Ferritina','Zinco','Selênio','Magnésio',
      'Vitamina D (25-OH)','Vitamina B12','PCR Ultrassensível','Homocisteína');
  END IF;
END $$;
