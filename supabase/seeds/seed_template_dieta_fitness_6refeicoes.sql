-- ─────────────────────────────────────────────────────────────────────────────
-- Seed: Template "Dieta Fitness — Alta Proteína 6 Refeições"
...
VALUES (
  'Dieta Fitness — Alta Proteína 6 Refeições',
  'Plano com 6 refeições ao longo do dia, priorizando fontes proteicas magras (frango, carne, ovos), carboidratos de baixo índice glicêmico e vegetais. Indicado para recomposição corporal e emagrecimento com preservação de massa muscular.',
  'alta_proteina',
...
-- ── STEP 2: Refeições ─────────────────────────────────────────────────────────
WITH tmpl AS (
  SELECT id FROM diet_templates WHERE name = 'Dieta Fitness — Alta Proteína 6 Refeições' LIMIT 1
)
...
WITH meal AS (
  ...
  WHERE dt.name = 'Dieta Fitness — Alta Proteína 6 Refeições' AND dtm.time_suggestion = '07:00' LIMIT 1
)
...
VALUES
  -- TACO #437: Batata-doce cozida sem sal — 86 kcal, P 1.4, C 20.2, G 0.1
  ((SELECT id FROM meal), 'Batata doce cozida sem sal', 100.0, 'g',  NULL,     NULL::numeric,  86.0,  1.4, 20.2, 0.1, 'Carboidrato', 0),
  -- TACO #15: Café coado sem açúcar — ~2 kcal/100mL
...
WITH meal AS (
  SELECT dtm.id FROM diet_template_meals dtm
  JOIN diet_templates dt ON dt.id = dtm.template_id
  WHERE dt.name = 'Dieta Fitness — Alta Proteína 6 Refeições' AND dtm.time_suggestion = '08:00' LIMIT 1
)
INSERT INTO diet_template_foods
  (template_meal_id, food_name, quantity, unit, household_measure, measure_amount,
   kcal_per_100g, protein_per_100g, carbs_per_100g, fat_per_100g, food_group, order_index)
VALUES
  -- Estimativa: omelete de claras recheado — 104 kcal, P 15.0, C 1.7, G 3.8
  ((SELECT id FROM meal), 'Omelete de claras recheado', 120.0, 'g', NULL, NULL::numeric, 104.0, 15.0, 1.7, 3.8, 'Proteína', 0),
  -- TACO #190: Frango peito cozido — 159 kcal, P 32.0, C 0.0, G 3.0
  ((SELECT id FROM meal), 'Frango em pedaços',           80.0, 'g', NULL, NULL::numeric, 159.0, 32.0, 0.0, 3.0, 'Proteína', 1),
...
WITH meal AS (
  SELECT dtm.id FROM diet_template_meals dtm
  JOIN diet_templates dt ON dt.id = dtm.template_id
  WHERE dt.name = 'Dieta Fitness — Alta Proteína 6 Refeições' AND dtm.time_suggestion = '12:00' LIMIT 1
)
INSERT INTO diet_template_foods
  (template_meal_id, food_name, quantity, unit, household_measure, measure_amount,
   kcal_per_100g, protein_per_100g, carbs_per_100g, fat_per_100g, food_group, order_index)
VALUES
  ...
  ((SELECT id FROM meal), 'Mandioquinha',                                  80.0, 'g', NULL,      NULL::numeric,  90.0,  2.3, 20.5,  0.3, 'Carboidrato', 0),
  -- TACO #29: Patinho grelhado s/ gordura — 219 kcal, P 30.7, C 0.0, G 10.0
  ((SELECT id FROM meal), 'Carne bovina, patinho, sem gordura, grelhado', 100.0, 'g', NULL,      NULL::numeric, 219.0, 30.7,  0.0, 10.0, 'Proteína',    1),
  -- TACO #335: Tomate cru — 18 kcal, P 0.9, C 3.5, G 0.2 (1 unidade ≈ 100g)
  ((SELECT id FROM meal), 'Tomate orgânico',                              100.0, 'g', 'unidade', 1.0::numeric,   18.0,  0.9,  3.5,  0.2, 'Vegetal',     2),
...
WITH meal AS (
  SELECT dtm.id FROM diet_template_meals dtm
  JOIN diet_templates dt ON dt.id = dtm.template_id
  WHERE dt.name = 'Dieta Fitness — Alta Proteína 6 Refeições' AND dtm.time_suggestion = '15:30' LIMIT 1
)
INSERT INTO diet_template_foods
  (template_meal_id, food_name, quantity, unit, household_measure, measure_amount,
   kcal_per_100g, protein_per_100g, carbs_per_100g, fat_per_100g, food_group, order_index)
VALUES
  ...
  -- TACO #225: Maçã com casca — 56 kcal, P 0.3, C 14.9, G 0.1 (1 unidade ≈ 130g)
  ((SELECT id FROM meal), 'Maçã',  130.0, 'g', 'unidade', 1.0::numeric,  56.0, 0.3, 14.9, 0.1, 'Fruta', 1),
  -- TACO #74: Banana prata — 98 kcal, P 1.3, C 25.8, G 0.1 (1 unidade ≈ 100g)
...
WITH meal AS (
  SELECT dtm.id FROM diet_template_meals dtm
  JOIN diet_templates dt ON dt.id = dtm.template_id
  WHERE dt.name = 'Dieta Fitness — Alta Proteína 6 Refeições' AND dtm.time_suggestion = '19:30' LIMIT 1
)
INSERT INTO diet_template_foods
  (template_meal_id, food_name, quantity, unit, household_measure, measure_amount,
   kcal_per_100g, protein_per_100g, carbs_per_100g, fat_per_100g, food_group, order_index)
VALUES
  -- Estimativa: sopa de legumes com carne — 50 kcal/100g, P 3.5, C 5.5, G 1.2
  ((SELECT id FROM meal), 'Sopa (legumes, carne)',       200.0, 'g', NULL, NULL::numeric,  50.0, 3.5,  5.5, 1.2, 'Outro',       0),
  -- TACO #437: Batata-doce cozida sem sal
...
WITH meal AS (
  SELECT dtm.id FROM diet_template_meals dtm
  JOIN diet_templates dt ON dt.id = dtm.template_id
  WHERE dt.name = 'Dieta Fitness — Alta Proteína 6 Refeições' AND dtm.time_suggestion = '23:30' LIMIT 1
)
INSERT INTO diet_template_foods
  (template_meal_id, food_name, quantity, unit, household_measure, measure_amount,
   kcal_per_100g, protein_per_100g, carbs_per_100g, fat_per_100g, food_group, order_index)
...
WHERE dt.name = 'Dieta Fitness — Alta Proteína 6 Refeições'
